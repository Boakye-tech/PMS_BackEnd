...
namespace Modules.Customers.Application.UseCases
{
	public class CustomerPaymentsService: ICustomerPaymentsService
    {
        ...
        {
            var customerPayments = (await _unitOfWork.CustomerPayment.GetAll(p => p.PropertyNumber == propertyNumber && p.DateOfPayment >= startDate && p.DateOfPayment <= endDate || p.PaymentMode!.Contains(paymentMode)))
                            .OrderByDescending(t => t.DateOfPayment);
